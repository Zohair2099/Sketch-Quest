...
import {z} from 'zod';
import {getUsers, getInstitutionByName} from '@/lib/user-data';
...
const getStudentInfo = ai.defineTool(
  {
    name: 'getStudentInfo',
    description:
      "Get information about students, such as their experience points (XP) or quest completion status. Use this to answer questions about student rankings or who has finished a specific quest. You can also filter by school/college name.",
    inputSchema: z.object({
      ...
      institutionName: z.string().optional().describe('The name of the school or college to filter by, e.g., "SketchQuest University".'),
    }),
  ...
  async (input) => {
    ...
    if (input.institutionName) {
        const institution = await getInstitutionByName(input.institutionName);
        if (institution) {
            institutionId = institution.id;
        } else {
            // If the institution is not found, we can return an empty list or let the model know.
            // Returning an empty list is simpler for now.
            return { students: [] };
        }
    }
...
If the user asks for a list of students based on their performance, like who has the most XP or who has completed a specific quest, use the getStudentInfo tool to fetch the data and then present it in a friendly, readable format. You can also filter these results by school if the user specifies one.
